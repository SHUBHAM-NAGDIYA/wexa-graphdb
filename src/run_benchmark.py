...
def main(platform: str):
    if platform not in PLATFORM_FACTORIES:
        print(f"Unknown platform '{platform}'. Choose from: {list(PLATFORM_FACTORIES)}")
        sys.exit(1)

    factory = PLATFORM_FACTORIES[platform]
    results = {"platform": platform, "run_started_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())}
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python src/run_benchmark.py <platform>")
        sys.exit(1)
    main(sys.argv[1])
